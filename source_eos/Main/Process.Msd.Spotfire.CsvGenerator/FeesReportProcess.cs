...
{
    public class FeesReportProcess : Ejecucion, IProcess
    {
        ...
        private FeesReportCsvGenerator _feesReportCsvGenerator { get; set; }

        public FeesReportProcess(Resultado result)
        {
            _result = result;
            _sqlDataManager = new SqlDataManager(this, _result);
            _emailManager = new EmailManager(this, _result);
            _feesReportCsvGenerator = new FeesReportCsvGenerator(this, _result);
        }
        ...
        protected override void Run()
        {
            ...
            dataSet = _sqlDataManager.RunStoreProcedureWithResult("sp_spotfire_generate_csv_fees");
            ...
            if (dataSet != null && dataSet.Tables.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(generatedFilePath))
                {
                    ...
                    if (_emailManager.SendEmail("[EOS MSD - Spotfire] CSV Exportación honorarios", "Registros a procesar: " + dataSet.Tables[0].Rows.Count + "\n Archivo generado en: " + generatedFilePath))
                    {
        ...
        private string GenerateCsv(DataTable dataTable)
        {
            try
            {
                ...
                var generatedFilePath = _feesReportCsvGenerator.GenerateCsv(dataRows);
        ...
        public void WriteLineApp(string message, int level)
        {
            if (level == -1)
            {
                AddMensajeAndWriteLineLog(DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + " [INF_FEES] " + message, level);
            }
            else
            {
                WriteLine(DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + " [INF_FEES] " + message, level);
            }
            Console.WriteLine(DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + " [INF_FEES] " + message);
        }
    }
}