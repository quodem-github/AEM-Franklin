...
{
    public class DossierReportProcess : Ejecucion, IProcess
    {
        ...
        private DossierReportCsvGenerator _dossierReportCsvGenerator { get; set; }

        public DossierReportProcess(Resultado result)
        {
            _result = result;
            _sqlDataManager = new SqlDataManager(this, _result);
            _emailManager = new EmailManager(this, _result);
            _dossierReportCsvGenerator = new DossierReportCsvGenerator(this, _result);
        }
        ...
        protected override void Run()
        {
            ...
            WriteLineApp("Proceso de informe de expedientes", -1);
            WriteLineApp("Iniciado proceso...", -1);
            WriteLineApp("Procesando datos...", -1);

            DataSet dataSet = null;

            dataSet = _sqlDataManager.RunStoreProcedureWithResult("sp_spotfire_generate_csv_dossier");
            ...
            if (dataSet != null && dataSet.Tables.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(generatedFilePath))
                {
                    ...
                    if (_emailManager.SendEmail("[EOS MSD - Spotfire] CSV Exportación expedientes", "Registros a procesar: " + dataSet.Tables[0].Rows.Count + "\n Archivo generado en: " + generatedFilePath))
                    {
        ...
        private string GenerateCsv(DataTable dataTable)
        {
            try
            {
                ...
                var generatedFilePath = _dossierReportCsvGenerator.GenerateCsv(dataRows);
        ...
        public void WriteLineApp(string message, int level)
        {
            if (level == -1)
            {
                AddMensajeAndWriteLineLog(DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + " [INF_EXP] " + message, level);
            }
            else
            {
                WriteLine(DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + " [INF_EXP] " + message, level);
            }
            Console.WriteLine(DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + " [INF_EXP] " + message);
        }
    }
}