...
{
    public class PermisoLegal : IPermiso
    {
        public EstadosAmec Estado { get { return EstadosAmec.PendienteDtoLegal; } }
        private readonly AgenteAprobadorAmec _agAprobador;

        public PermisoLegal(AgenteAprobadorAmec agAprobador)
        {
        ...
        public bool PuedoAprobarYRechazar(string idamec, int idAprobador)
        {
            ...
            return agAprobador.ObtenerPermisosLegal(idamec, idAprobador);
        }
        ...
        public bool Rechazar(string idamec, int idPeticionario)
        {
            ...
            try
            {
                ...
                agAmec.InsertarHistorialAmec(EstadosAmec.Rechazado.GetHashCode(), idamec, usuario, NivelesAprobacion.DepartamentoLegal.GetHashCode(), null);
        ...
        public bool Aprobar(string idamec, int idPeticionario, bool condicionado)
        {
            AgenteAmecInfo agAmec = new AgenteAmecInfo();
            DAmecInfo amec = agAmec.CargarTodosValoresAmec(idamec);
            EstadosAmec tipoEstado = condicionado ? EstadosAmec.AprobadoCondicionadoDtoLegal : EstadosAmec.AprobadoDtoLegal;
            try
            {
                ...
                agAmec.InsertarHistorialAmec(tipoEstado.GetHashCode(), idamec, usuario, NivelesAprobacion.DepartamentoLegal.GetHashCode(), null);
            ...
            {
                agAmec.BorrarHistorialAmec(tipoEstado.GetHashCode(), idamec, idPeticionario, NivelesAprobacion.DepartamentoLegal.GetHashCode());
                agAmec.BorrarHistorialAmec(EstadosAmec.Aprobado.GetHashCode(), idamec, idPeticionario, NivelesAprobacion.Aprobado.GetHashCode());
                agAmec.CambiarEstadoAmec(amec.idestado.ToString(), idamec.ToString(), amec.idnivelaprobacion);
                throw;
            }
        }
        ...
        {
            return _agAprobador.HeAprobadoLegal(idamec);
        }
        public List<string> ObtenerListaDestinatarios(string idamec, int idPeticionario)
        {
            ...
            {
                AgenteAprobadorAmec agAprobador = new AgenteAprobadorAmec();
                return agAprobador.DestinatariosLegal().Select(x => x.ToString()).ToList();
            }
            catch (Exception)
            {
                throw;
