...
namespace EOS.Entidades.Datos
{
    public class DVCongresoAmec
    {
        ...
        private Int32 m_idameccongreso;
        //[PropiedadOriginal("idameccongreso", TablaOriginal = "", TipoProveedor = ClepsydraDbType.Int32, Longitud = 11)]
        public Int32 idameccongreso
        {
            get { return m_idameccongreso; }
            set
            {
                if (this.m_idameccongreso != value)
                {
                    //OnNotificaCambiosPropiedad(new NotificaCambiosPropiedadEventArgs("idameccongreso", m_idameccongreso, value));
                    m_idameccongreso = value;
        ...
        public static ICollection<DVCongresoAmec> ConvertToDto(DataTable dtTable)
        {
            ICollection<DVCongresoAmec> collection = new Collection<DVCongresoAmec>();

            foreach (DataRow row in dtTable.Rows)
            {
                DVCongresoAmec data = new DVCongresoAmec()
                {
                    ...
                    idameccongreso = Quodem.Utility.DataLayerUtil.GetIntValue(row, "idameccongreso"),
