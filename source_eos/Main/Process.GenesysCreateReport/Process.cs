...
using msd.GenesysCreateReportProcess.Class.DataAccess;
using msd.GenesysCreateReportProcess.Class.EmailManagement;
using msd.GenesysCreateReportProcess.Class.FileManagement;
...
namespace msd.GenesysCreateReportProcess
{
    public class Process : Ejecucion
    {
        ...
        protected override void Run()
        {
            ...
            var firstdateyear = new DateTime();
            var lastdateyear = new DateTime();
            ...
            if (dateTimeCollection.Any())
            {
                ...
                if (dateTimeCollection.Count > 2)
                {
                    firstdateyear = dateTimeCollection[2];
                }
                else 
                { 
                    firstdateyear = dateTimeCollection[0];
                }

                if (dateTimeCollection.Count > 3)
                {
                    lastdateyear = dateTimeCollection[3];
                }
                else
                {
                    lastdateyear = dateTimeCollection[1];
                }

                SqlDataManager.RunStoreProcedureNoResult(AppConfigs.StoreProcedurePrepare);

                dataSet = ProcessData(startDateTime, endDateTime, firstdateyear, lastdateyear);
        ...
        private DataSet ProcessData(DateTime startDateTime, DateTime endDateTime, DateTime firstDayYear, DateTime lastDayYear)
        {
            var rowsCount = 0;
            try
            {
                var parametersCollection = new List<SqlParameter>
                {
                    ...
                    new SqlParameter("p_first_day_year", firstDayYear),
                    new SqlParameter("p_last_day_year", lastDayYear)
                };
        ...
        private List<DateTime> GetPeriod(DateTime date)
        {
            ...
            if (dsDates != null && dsDates.Tables.Count > 0 && dsDates.Tables[0].Rows.Count > 0)
            {
                ...
                if (row["firstdateyear"] != DBNull.Value)
                {
                    dates.Add(DateTime.Parse(row["firstdateyear"].ToString()));
                }
                if (row["lastdateyear"] != DBNull.Value)
                {
                    dates.Add(DateTime.Parse(row["lastdateyear"].ToString()));
                }
