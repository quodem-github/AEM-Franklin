...
namespace EOS.Api
{
	public class ServiciosReservasTransporteController : ApiController
    {

        private IService _service;
        private readonly AgencyKeyService _agencyKeyService;

        public ServiciosReservasTransporteController()
        {
        ...
        public HttpResponseMessage GetList([FromBody] ReceivedData encryptedFilter)
        {
            ...
            try
            {
                ...
                _service = new ServiciosReservasTransporteService(token, agencyKey);
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio ServiciosReservasTransporteController.GetList " + ex.Message + " - " + ex.StackTrace);
        ...
        public HttpResponseMessage Edit([FromBody] ReceivedData encryptedFilter)
        {
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio ServiciosReservasTransporteController.Edit " + ex.Message + " - " + ex.StackTrace);
                result.AddError(ErrorCode.DataNotValid,
                    Utility.GetDisplayName(typeof(ErrorCode), ErrorCode.DataNotValid.ToString()));
