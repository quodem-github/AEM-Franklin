...
namespace EOS.Api
{
    public class EstadosReservasController : ApiController
    {
        private IService _service;
        private readonly AgencyKeyService _agencyKeyService;

        public EstadosReservasController()
        {
        ...
        public HttpResponseMessage GetList([FromBody] ReceivedData encryptedFilter)
        {
            ...
            try
            {
                ...
                _service = new EstadosReservasService(token, agencyKey);
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio EstadosReservasController.GetList " + ex.Message + " - " + ex.StackTrace);
        ...
        public HttpResponseMessage Edit([FromBody] ReceivedData encryptedFilter)
        {
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio EstadosReservasController.Edit " + ex.Message + " - " + ex.StackTrace);
                result.AddError(ErrorCode.DataNotValid,
                    Utility.GetDisplayName(typeof(ErrorCode), ErrorCode.DataNotValid.ToString()));
