...
namespace EOS.Api
{
    public class TiposActividadCongresoController : ApiController
    {
        private IServiceBase _service;
        private readonly AgencyKeyService _agencyKeyService;

        public TiposActividadCongresoController()
        {
        ...
        public HttpResponseMessage GetList([FromBody] ReceivedData encryptedFilter)
        {
            ...
            try
            {
                ...
                _service = new TiposActividadCongresoService(token, agencyKey);
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio TiposActividadCongresoController.GetList " + ex.Message + " - " + ex.StackTrace);
