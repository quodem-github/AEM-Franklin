...
namespace EOS.Api
{
    public class ServiciosReservasHotelController : ApiController
    {

        private IService _service;
        private readonly AgencyKeyService _agencyKeyService;

        public ServiciosReservasHotelController()
        {
        ...
        public HttpResponseMessage GetList([FromBody] ReceivedData encryptedFilter)
        {
            ...
            try
            {
                ...
                _service = new ServicioReservaHotelService(token, agencyKey);
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio ServiciosReservasHotelController.GetList " + ex.Message + " - " + ex.StackTrace);
        ...
        public HttpResponseMessage Edit([FromBody] ReceivedData encryptedFilter)
        {
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio ServiciosReservasHotelController.Edit " + ex.Message + " - " + ex.StackTrace);
                result.AddError(ErrorCode.DataNotValid,
                    Utility.GetDisplayName(typeof(ErrorCode), ErrorCode.DataNotValid.ToString()));
