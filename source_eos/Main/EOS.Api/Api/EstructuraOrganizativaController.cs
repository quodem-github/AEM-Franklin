...
namespace EOS.Api
{
    public class EstructuraOrganizativaController : ApiController
    {
        private IServiceBase _service;
        private readonly AgencyKeyService _agencyKeyService;

        public EstructuraOrganizativaController()
        {
        ...
        public HttpResponseMessage GetList([FromBody] ReceivedData encryptedFilter)
        {
            ...
            try
            {
                ...
                _service = new EstructuraOrganizativaService(token, agencyKey);
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio EstructuraOrganizativaController.GetList " + ex.Message + " - " + ex.StackTrace);
