...
namespace EOS.Api
{
    public class ReservasViajesController : ApiController
    {

        private IService _service;
        private readonly AgencyKeyService _agencyKeyService;

        public ReservasViajesController()
        {
        ...
        public HttpResponseMessage GetList([FromBody] ReceivedData encryptedFilter)
        {
            ...
            try
            {
                ...
                _service = new ReservaViajesService(token, agencyKey);
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio ReservasViajesController.GetList " + ex.Message + " - " + ex.StackTrace);
        ...
        public HttpResponseMessage Edit([FromBody] ReceivedData encryptedFilter)
        {
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio ReservasViajesController.Edit " + ex.Message + " - " + ex.StackTrace);
                result.AddError(ErrorCode.DataNotValid,
                    Utility.GetDisplayName(typeof(ErrorCode), ErrorCode.DataNotValid.ToString()));
