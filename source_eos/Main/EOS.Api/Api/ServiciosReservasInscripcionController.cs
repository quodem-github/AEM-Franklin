...
namespace EOS.Api
{
    public class ServiciosReservasInscripcionController : ApiController
    {

        private IService _service;
        private readonly AgencyKeyService _agencyKeyService;

        public ServiciosReservasInscripcionController()
        {
        ...
        public HttpResponseMessage GetList([FromBody] ReceivedData encryptedFilter)
        {
            ...
            try
            {
                ...
                _service = new ServiciosReservasInscripcionService(token, agencyKey);
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio ServiciosReservasInscripcionController.GetList " + ex.Message + " - " + ex.StackTrace);
        ...
        public HttpResponseMessage Edit([FromBody] ReceivedData encryptedFilter)
        {
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio ServiciosReservasInscripcionController.Edit " + ex.Message + " - " + ex.StackTrace);
                result.AddError(ErrorCode.DataNotValid,
                    Utility.GetDisplayName(typeof(ErrorCode), ErrorCode.DataNotValid.ToString()));
