...
namespace EOS.Api
{
    public class AprobadorAmecController : ApiController
    {
        private IServiceBase _service;
        private readonly AgencyKeyService _agencyKeyService;

        public AprobadorAmecController()
        {
        ...
        public HttpResponseMessage GetList([FromBody] ReceivedData encryptedFilter)
        {
            ...
            try
            {
                ...
                _service = new AprobadorAmecService(token, agencyKey);
            ...
            {
                Quodem.Monitor.Alerta.WriteLog("Error servicio AprobadorAmecController.GetList " + ex.Message + " - " + ex.StackTrace);
