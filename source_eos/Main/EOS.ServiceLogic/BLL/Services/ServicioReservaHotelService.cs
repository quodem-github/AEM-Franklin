...
{
    public class ServicioReservaHotelService : IService
    {
        #region Definitions
        ...
        private FilterServicioReservasHotel _filter;
        private ServiciosReservasHotelEditDto _data;
        private readonly string _token;
        private readonly string _agencyKey;
        private QuodemLogService _logService;
        #endregion

        public ServicioReservaHotelService(string token, string agencyKey)
        {
        ...
        public ServiceError ValidFilter(QSuscriptor suscriptor, string data)
        {
            ...
            var validateService = new ValidateService<FilterServicioReservasHotel>
            {
                DateFromProp = "",
                DateToProp = "",
                DateTimeFromProp = "",
        ...
        public string GetList(QSuscriptor suscriptor)
        {
            var query = _session.Query<Expediente>()
                ...
                .Join(_session.Query<Serviciosreservasviajes>(), re => new { IdReserva = re.Idreserva },
                    ...
                    (re, srv) => new { srv.Idservicio, re.Idreserva, re.Idxpediente, re.Idagencia, srv.Idserviciohotel })
                .Join(_session.Query<Serviciosreservashotel>(), srv => new { IdServicioHotel = srv.Idserviciohotel ?? Variables.NULL_INT },
                    srh => new { IdServicioHotel = srh.Idserviciohotel },
            ...
            {
                query = query.Where(x => x.srh.Idserviciohotel == _filter.IdServicioHotel.Value);
            }

            var list =
                query.Skip((_filter.CurrentPageIndex - 1 ?? 0) * Variables.ServicePageSize)
                    .Take(Variables.ServicePageSize)
                    .Select(x => Mapper.Map<ServiciosReservasHotelDto>(x.srh)).ToList();

            int rowCount = query.Count();

            var result = new ServiciosReservasHotelResponseDto()
            {
                ServiciosReservasHotelList = list,
            ...
            _logService.AddLogEntry("ServicioReservaHotel", JsonConvert.SerializeObject(_filter), rowCount.ToString(), _agencyKey, false);
        ...
        public ServiceError ValidData(QSuscriptor suscriptor, string data)
        {
            ...
            var validateService = new ValidateService<ServiciosReservasHotelEditDto>();
        ...
        public SrvResponse Edit(QSuscriptor suscriptor)
        {
            ...
            using (var transaction = _session.BeginTransaction())
            {
                try
                {
                    var query = _session.Query<Serviciosreservashotel>()
                .Join(_session.Query<Confempresa>(),
                    srh => new { IdConf = srh.Idconfempresa },
                    co => new { IdConf = co.Idconfempresa },
                    (am, co) => new { co.Idconfempresa, co.Idagencia, am.Idserviciohotel })

                .Where(x => x.Idagencia == _agencyKey);

                    if (_data.Idserviciohotel != Variables.NULL_INT)
                    {
                        query = query.Where(x => x.Idserviciohotel == _data.Idserviciohotel);
                    }
                    ...
                    if (!isAgencyService)
                    {
                        ...
                        {
                            Code = ErrorCode.ServiciosReservasHotelInsertUpdateError.GetHashCode(),
                            Description = Utility.GetDisplayName(typeof(ErrorCode), ErrorCode.ServiciosReservasHotelInsertUpdateError.ToString())
                        });

                        //Generar el log en la tabla de auditoria
                        _logService.AddLogEntry("ServiciosReservasHotel", JsonConvert.SerializeObject(_data), "Error en ServicioReservaHotelService.Edit: El Idconfempresa no pertenece a la agencia que está usando el servicio", _agencyKey);
                        transaction.Commit();
                        return result;
                    }
                    ...
                    //Generar el log en la tabla de auditoria
                    _logService.AddLogEntry("ServiciosReservasHotel", JsonConvert.SerializeObject(_data),
                        JsonConvert.SerializeObject(editData), _agencyKey);

                    //Generar la respuesta cuando se ha actualizado o insertado correctamente.
                    var response = new ServiciosReservasHotelEditResponseDto()
                    {
                        ServiciosReservasHotel = _data,
                        ServiciosReservasHotelResult = Mapper.Map<ServiciosReservasHotelEditDto>(editData)
                    };
                ...
                catch (Exception e)
                {
                    ...
                    Quodem.Monitor.Alerta.SendNotificacion("Error en ServicioReservaHotelService.Edit: " + e.Message);
                    result.ErrorList.Add(new ErrorItem()
                    {
                        Code = ErrorCode.ServiciosReservasHotelInsertUpdateError.GetHashCode(),
                        Description =
                            Utility.GetDisplayName(typeof(ErrorCode),
                                ErrorCode.ServiciosReservasHotelInsertUpdateError.ToString())
                    });
                    transaction.Rollback();
                    _logService.AddLogEntry("ServiciosReservasHotel", JsonConvert.SerializeObject(_data),
                        "Error en ServicioReservaHotelService.Edit: " + e.Message, _agencyKey);
