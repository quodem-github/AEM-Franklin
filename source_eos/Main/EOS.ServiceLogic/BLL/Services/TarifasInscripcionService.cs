...
{
    public class TarifasInscripcionService : IService
    {
        #region Definitions
        ...
        #region Properties
        private TarifasInscripcionEditDto _data;
        private FilterTarifasInscripcion _filter;
        private readonly string _token;
        private readonly string _agencyKey;
        private QuodemLogService _logService;
        #endregion

        public TarifasInscripcionService(string token, string agencyKey)
        {
        ...
        public ServiceError ValidFilter(QSuscriptor suscriptor, string data)
        {
            ...
            var validateService = new ValidateService<FilterTarifasInscripcion>
            {
                DateFromProp = "",
                DateToProp = "",
                DateTimeFromProp = "",
        ...
        {
            var query = _session.Query<Tarifasinscripcion>()
            ...
            {
                query = query.Where(x => x.tam.tar.Fkidcongreso == _filter.IdCongreso.Value);
            }

            if (_filter.IdTarifaInscripcion != Variables.NULL_INT)
            {
                query = query.Where(x => x.tam.tar.Idtarifainscripcion == _filter.IdTarifaInscripcion.Value);
            }

            var TarifasIncripciones =
                query.Skip((_filter.CurrentPageIndex.Value - 1) * Variables.ServicePageSize)
                    .Take(Variables.ServicePageSize)
                    .Select(x => Mapper.Map<TarifasInscripcionDto>(x.tam.tar)).ToList();

            int rowCount = query.Count();

            var result = new TarifasInscripcionResponseDto()
            {
                TarifasInscripcionList = TarifasIncripciones,
                CurrentPageIndex = _filter.CurrentPageIndex,
                PageSize = Variables.ServicePageSize,
                IdCongreso = _filter.IdCongreso,
                IdTarifaInscripcion = _filter.IdTarifaInscripcion,
                PageCount = (int)Math.Ceiling(rowCount * 1.0 / Variables.ServicePageSize)
            };

            _logService.AddLogEntry("TarifasInscripcion", JsonConvert.SerializeObject(_filter), rowCount.ToString(), _agencyKey, false);
        ...
        public ServiceError ValidData(QSuscriptor suscriptor, string data)
        {
            ...
            var validateService = new ValidateService<TarifasInscripcionEditDto>();
        ...
        public SrvResponse Edit(QSuscriptor suscriptor)
        {
            ...
            using (var transaction = _session.BeginTransaction())
            {
                ...
                .Where(x => x.conf.Idagencia == _agencyKey && x.tam.Idcongreso == _data.Fkidcongreso);
                    ...
                    List<Tarifasinscripcion> tarifasList = null;
                    if (isAgencyService && _data.Idtarifainscripcion != Variables.NULL_INT)
                    {
                        tarifasList = _session.Query<Tarifasinscripcion>().Where(x => x.Idtarifainscripcion == _data.Idtarifainscripcion).ToList();
                    ...
                    if (!isAgencyService)
                    {
                        ...
                        {
                            Code = ErrorCode.TarifasInscripcionInsertUpdateError.GetHashCode(),
                            Description = Utility.GetDisplayName(typeof(ErrorCode), ErrorCode.TarifasInscripcionInsertUpdateError.ToString())
                        });

                        //Generar el log en la tabla de auditoria
                        _logService.AddLogEntry("Tarifasinscripcion", JsonConvert.SerializeObject(_data), "Error en TarifasInscripcionService.Edit: El Idcongreso no pertenece a la agencia que está usando el servicio", _agencyKey);
                        transaction.Commit();
                        return result;
                    }
                    ...
                    var editData = Mapper.Map<Tarifasinscripcion>(_data);
                    //Si el Id es null o -1 entonces se inserta, si no, se actualiza.
                    if (editData.Idtarifainscripcion == Variables.NULL_INT)
                    {

                        //Obtener el ultimo ID para realizar la inserción en los casos que no tenga autonumerico la tabla destino.
                        var lastId = _session.Query<Tarifasinscripcion>()
                            .Max(x => x.Idtarifainscripcion);
                        editData.Idtarifainscripcion = lastId + 1;
                    ...
                    //Generar el log en la tabla de auditoria
                    _logService.AddLogEntry("Tarifasinscripcion", JsonConvert.SerializeObject(_data),
                        JsonConvert.SerializeObject(editData), _agencyKey);

                    //Generar la respuesta cuando se ha actualizado o insertado correctamente.
                    var response = new TarifasInscripcionEditResponseDto()
                    {
                        TarifasInscripcion = _data,
                        TarifasInscripcionResult = Mapper.Map<TarifasInscripcionEditDto>(editData)
                    };
                ...
                catch (Exception e)
                {
                    ...
                    Quodem.Monitor.Alerta.SendNotificacion("Error en TarifasInscripcionService.Edit: " + e.Message);
                    result.ErrorList.Add(new ErrorItem()
                    {
                        Code = ErrorCode.TarifasInscripcionInsertUpdateError.GetHashCode(),
                        Description =
                            Utility.GetDisplayName(typeof(ErrorCode),
                                ErrorCode.TarifasInscripcionInsertUpdateError.ToString())
                    });
                    transaction.Rollback();
                    _logService.AddLogEntry("Tarifasinscripcion", JsonConvert.SerializeObject(_data),
                        "Error en TarifasInscripcionService.Edit: " + e.Message, _agencyKey);
